...
export function SubmissionModal(props: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-navy-950/80 p-4 backdrop-blur-sm sm:items-center sm:p-6"
      onClick={onClose}
    >
      <div
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full text-white/50 transition-colors duration-200 hover:bg-white/10 hover:text-white"
        >
          <X size={16} />
        </button>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-sky-400 font-mono text-base font-semibold text-white">
          {initials(c.name)}
        </div>
        <div>
...
function DiscoveryDetail({
  ...
  return (
    <div>
      ...
      <div className="mb-8 rounded-xl border border-white/10 bg-navy-950/50 p-5">
        <h3 className="mb-5 text-xs font-medium uppercase tracking-wider text-white/40">
          Project Roadmap
        </h3>
        <RoadmapStepper draft={draftStatus} onSelect={setDraftStatus} />

        <label className="mb-2 mt-6 block text-xs font-medium uppercase tracking-wider text-white/40">
          Internal Notes
        </label>
        <textarea
          value={draftNotes}
      ...
      <div className="flex flex-col gap-2.5 text-sm leading-relaxed text-white/60">
        ...
        {d.message && (
          <p className="mt-2 whitespace-pre-line text-white/70">{d.message}</p>
        )}
      </div>
    </div>
  );
